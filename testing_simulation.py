...
class Simulation:
    def __init__(self, Model, TrafficGen, sumo_cmd, max_steps, green_duration, yellow_duration, num_states, num_actions):
        ...
        self._actions_taken = {i: 0 for i in range(num_actions)}
        ...
        for car_id in car_list:
            ...
            if road_id in incoming_roads:  # consider only the waiting times of cars in incoming roads
                ...
                if car_id in self._waiting_times: # a car that was tracked has cleared the intersection
                    self._total_wait_time += self._waiting_times[car_id]
                    self._total_v += 1
    ...
    def _choose_action(self, state):
        ...
        """
        return np.argmax(self._Model.predict_one(state, verbose=0))
