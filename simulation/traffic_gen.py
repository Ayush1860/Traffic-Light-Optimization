...
class TrafficGenerator:
    ...
    def generate_routefile(self, seed):
        ...
        with open("intersection/episode_routes.rou.xml", "w") as routes:
            ...
            <route id="W_E" edges="W2TL1 TL12TL2 TL22TL3 TL32E"/>
            <route id="E_W" edges="E2TL3 TL32TL2 TL22TL1 TL12W"/>
            <route id="N1_S1" edges="N12TL1 TL12S1"/>
            <route id="S1_N1" edges="S12TL1 TL12N1"/>
            <route id="N2_S2" edges="N22TL2 TL22S2"/>
            <route id="S2_N2" edges="S22TL2 TL22N2"/>
            <route id="N3_S3" edges="N32TL3 TL32S3"/>
            <route id="S3_N3" edges="S32TL3 TL32N3"/>

            <route id="W_N1" edges="W2TL1 TL12N1"/>
            <route id="W_S1" edges="W2TL1 TL12S1"/>
            <route id="E_N3" edges="E2TL3 TL32N3"/>
            <route id="E_S3" edges="E2TL3 TL32S3"/>
            <route id="N1_E" edges="N12TL1 TL12TL2 TL22TL3 TL32E"/>
            <route id="S1_E" edges="S12TL1 TL12TL2 TL22TL3 TL32E"/>
            <route id="N3_W" edges="N32TL3 TL32TL2 TL22TL1 TL12W"/>
            <route id="S3_W" edges="S32TL3 TL32TL2 TL22TL1 TL12W"/>""", file=routes)

            for car_counter, step in enumerate(car_gen_steps):
                straight_or_turn = np.random.uniform()
                if straight_or_turn < 0.75:  # choose direction: straight or turn - 75% of times the car goes straight
                    route_straight = np.random.randint(1, 9)  # choose a random source & destination
                    if route_straight == 1:
                        ...
                        print('    <vehicle id="N3_S3_%i" type="standard_car" route="N3_S3" depart="%s" departLane="random" departSpeed="10" />' % (car_counter, step), file=routes)
                    else:
                        print('    <vehicle id="S3_N3_%i" type="standard_car" route="S3_N3" depart="%s" departLane="random" departSpeed="10" />' % (car_counter, step), file=routes)
                    ...
                    if route_turn == 1:
                        ...
                        print('    <vehicle id="E_N3_%i" type="standard_car" route="E_N3" depart="%s" departLane="random" departSpeed="10" />' % (car_counter, step), file=routes)
                    elif route_turn == 4:
                        print('    <vehicle id="E_S3_%i" type="standard_car" route="E_S3" depart="%s" departLane="random" departSpeed="10" />' % (car_counter, step), file=routes)
                    elif route_turn == 5:
                        ...
                        print('    <vehicle id="N3_W_%i" type="standard_car" route="N3_W" depart="%s" departLane="random" departSpeed="10" />' % (car_counter, step), file=routes)
                    elif route_turn == 8:
                        print('    <vehicle id="S3_W_%i" type="standard_car" route="S3_W" depart="%s" departLane="random" departSpeed="10" />' % (car_counter, step), file=routes)
